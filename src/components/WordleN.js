...
function WordleN({user, setWordLength, wordLength, uuid}) {
    ...
    const createGame = async (length) => {
        ...
        const fetchData = async () => {
            ...
            if (response.ok) {
                ...
                if (!!data.wordLength) {
                    setWordLength(Number(data.wordLength))
                    setDisabled2(true)
                    if (!!data.guesses) {
    ...
    const share = async () => {
        try {
            ...
            if (response.ok) {
                const data = await response.json();
                navigator.clipboard.writeText(`${window.location.origin}/sharedn/${data.uuid}`)
    ...
    useEffect(() => {
        ...
        if (window.location.pathname.startsWith("/sharedn/")) {
            createGame()
        }
    }, [])
        ...
        if (allowSend) {
            try {
                ...
                if (guess.length !== wordLength){
                    console.log("Guess is not the right length")
                ...
                if (response.ok) {
                    ...
                    else if (data.guessCount >= wordLength+1){
                        setIsLoss(true)
